...
Run:
    python -m research.run_dynamic_exposure_sweep
"""
...
def main() -> None:
    ...
    BASE_CFG = dict(
        ...
        market_sma_days=200,
        risk_on_buffer=0.0,
        risk_off_exposure=0.0,  # not used when full-cash path is bypassed by min_exposure > 0
    )

    dynamic_configs = [
        {"label": "fixed_025",            "min_exposure": 0.25, "max_exposure": 0.25, "exposure_slope": 0.0},
        {"label": "dynamic_conservative", "min_exposure": 0.25, "max_exposure": 1.0,  "exposure_slope": 3.0},
        {"label": "dynamic_balanced",     "min_exposure": 0.25, "max_exposure": 1.0,  "exposure_slope": 5.0},
        {"label": "dynamic_aggressive",   "min_exposure": 0.25, "max_exposure": 1.0,  "exposure_slope": 8.0},
    ]
    ...
    for dc in dynamic_configs:
        print(f"[INFO] Running walk-forward for {dc['label']}...")

        cfg = WalkForwardConfig(
            **BASE_CFG,
            min_exposure=dc["min_exposure"],
            max_exposure=dc["max_exposure"],
            exposure_slope=dc["exposure_slope"],
        )
