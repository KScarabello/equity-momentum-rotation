#!/usr/bin/env python3
"""
Run an SMA length sweep (with slope filter enabled) for the equity momentum strategy.

Tests whether a faster or slower SMA trend filter improves risk-adjusted returns
when combined with the positive-slope requirement.

Run:
    python -m research.run_sma_length_with_slope_sweep
"""
...
def _participation_ratio(equity_oos: pd.Series, threshold: float = 1e-4) -> float:
    """Fraction of days where strategy had non-trivial equity movement (proxy for invested)."""
    if len(equity_oos) < 2:
        return float("nan")
    rets = equity_oos.pct_change().dropna().abs()
...
def main() -> None:
    ...
    configs = [
        {"label": "sma_150_slope", "market_sma_days": 150},
        {"label": "sma_175_slope", "market_sma_days": 175},
        {"label": "sma_200_slope", "market_sma_days": 200},
        {"label": "sma_225_slope", "market_sma_days": 225},
        {"label": "sma_250_slope", "market_sma_days": 250},
    ]
    ...
    rows = []
    for c in configs:
        print(f"[INFO] Running walk-forward for {c['label']} (market_sma_days={c['market_sma_days']})...")

        cfg = WalkForwardConfig(**BASE_CFG, market_sma_days=c["market_sma_days"])
        ...
        participation = _participation_ratio(equity_oos)

        rows.append(
            {
                "label": c["label"],
                "market_sma_days": c["market_sma_days"],
