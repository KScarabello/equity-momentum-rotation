#!/usr/bin/env python3
"""
Run a stability filter sweep for the equity momentum strategy.

Tests whether requiring stocks to appear in top rankings across multiple
consecutive rebalances reduces noise and improves risk-adjusted returns.

Run:
    python -m research.run_stability_filter_sweep
"""
...
def main() -> None:
    ...
    for stability in stability_values:
        print(f"[INFO] Running walk-forward for stability_lookback_periods={stability}...")

        cfg = WalkForwardConfig(**BASE_CFG, stability_lookback_periods=stability)
